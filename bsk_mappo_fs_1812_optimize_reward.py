...
class BasiliskFullMissionEnv:
    ...
    def _init_simulator(self):
        ...
        if self.viz:
            try:
                viz = vizSupport.enableUnityVisualization(self.scSim, self.taskName, self.all_viz_objects, saveFile="mission_sim_optimize_reward.bin")
                    ...
                    if act in self.global_captured_targets: rewards[i] -= 0.5
                        ...
                        if not is_daylight:
                            ...
                            if angle <= max_turn:
                                ...
                                if dist <= 4800e3 and off_nadir <= 65.0:
                                    self.global_captured_targets.add(act)
                                    state['buffer'] += 1
                                    rewards[i] += 10.0
                                    if debug_mode: print(f"[t={self.sim_time:.0f}] 📸 Sat {i} CAPTURED T{act} (Daylight). GLOBAL TARGET: {len(self.global_captured_targets)}/{self.n_targets}")
                                else: 
                                    rewards[i] -= 0.1
