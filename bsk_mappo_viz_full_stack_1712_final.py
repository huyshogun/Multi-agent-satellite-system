            ...
            '''
                ...
                if angle <= max_turn:
                    ...
                    if state['buffer'] > 0:
                        if dist < 2500e3:
                            state['buffer'] -= 1
                            rewards[i] += 20.0 # Thưởng lớn
                            if debug_mode:
                                gs_names = ["Hanoi", "NY", "London", "Sydney"]
                                print(f"[t={self.sim_time:.0f}] 📡 Sat {i} DOWNLINK @ {gs_names[gs_idx]} -> +20đ")
                        else: rewards[i] -= 0.1 # Xa quá
                    else: rewards[i] -= 0.2 # Buffer trống
                else:
                    ratio = max_turn / angle
                    new_vec = (1 - ratio) * state['bore_vec'] + ratio * req_vec
                    state['bore_vec'] = new_vec / np.linalg.norm(new_vec)
                    if debug_mode: print(f"[t={self.sim_time:.0f}] 🔄 Sat {i} Slew GS")
